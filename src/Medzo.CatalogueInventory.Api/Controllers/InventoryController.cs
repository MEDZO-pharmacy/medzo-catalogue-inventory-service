...
public sealed class InventoryController(IInventoryService service):ControllerBase{
 ...
 [HttpGet("items/low-stock"),Authorize(Policy="InventoryManage")]public Task<PagedResult<InventoryResponse>> Low([FromQuery]string? search,[FromQuery]int page=1,[FromQuery]int pageSize=20,CancellationToken ct=default)=>service.GetAsync(search,true,page,pageSize,ct);
 ...
 [HttpGet("sale-issues")]public Task<PagedResult<SaleStockIssueResponse>> SaleIssues([FromQuery]int page=1,[FromQuery]int pageSize=20,CancellationToken ct=default)=>service.GetSaleIssuesAsync(page,pageSize,ct);
