...
public sealed class CatalogueController(ICatalogueService service):ControllerBase{
 [HttpGet("public"),AllowAnonymous]public Task<PagedResult<PublicMedicineResponse>> Public([FromQuery]string? search,[FromQuery]int page=1,[FromQuery]int pageSize=12,CancellationToken ct=default)=>service.BrowsePublicAsync(search,page,pageSize,ct);
 ...
 [HttpDelete("{id:guid}"),Authorize(Policy="InventoryManage")]public async Task<IActionResult> Delete(Guid id,[FromHeader(Name="If-Match")]long version,CancellationToken ct){await service.DeleteAsync(id,version,ct);return NoContent();}}
